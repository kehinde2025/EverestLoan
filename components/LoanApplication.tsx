        ...
        <div className="space-y-1">
            ...
            <div
                ...
                className={`cursor-pointer rounded-2xl border-2 p-6 flex flex-col items-center justify-center text-center transition-all duration-200
                    ${error ? "border-red-400 bg-red-50" : selectedFile ? "border-[#059669] bg-[#ECFDF5]" : "border-[#A7DCC8] bg-[#ECFDF5] hover:border-[#059669] hover:bg-[#D1FAE5]"}`}
            >
                <div className={`text-4xl mb-3 ${selectedFile ? "text-[#059669]" : "text-[#A7DCC8]"}`}>
                    {selectedFile ? (
                ...
                <p className={`text-xs ${selectedFile ? "text-[#059669] font-semibold" : "text-gray-400"}`}>
...
export default function LoanApplication() {
        ...
        return (
            <div className="max-w-3xl mx-auto p-6">
                <motion.div
                    ...
                    className="bg-white shadow-lg border border-[#A7DCC8] p-10 text-center rounded-sm"
                >
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ delay: 0.2, type: "spring", stiffness: 200, damping: 15 }}
                        className="w-24 h-24 rounded-full bg-[#D1FAE5] border-4 border-[#059669] flex items-center justify-center mx-auto mb-6"
                    >
                        <motion.svg
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                        <span className="inline-flex items-center justify-center gap-2 text-xs font-semibold px-4 py-2 rounded-full mb-6 bg-[#D4A63A]/90 text-[#0F172A] shadow-sm">
                            APPLICATION RECEIVED
                        </span>
                        <h2 className="text-3xl font-bold text-[#064E3B] mb-3">You&apos;re All Set!</h2>
                        ...
                        <p className="text-gray-500 text-sm mb-8">
                            ...
                            <span className="font-semibold text-[#064E3B]">24–48 hours</span>{" "}
                            via the email or phone number you provided.
                        </p>
                        <div className="bg-[#ECFDF5] border border-[#A7DCC8] rounded-lg p-4 mb-8 text-left space-y-2">
                            <h3 className="font-semibold text-[#022C22] text-sm uppercase tracking-wide mb-3">What happens next?</h3>
                            {[
                                ...
                                <div key={item.step} className="flex items-center gap-3">
                                    <span className="w-6 h-6 rounded-full bg-[#059669] text-white text-xs flex items-center justify-center font-bold flex-shrink-0">{item.step}</span>
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg border border-[#A7DCC8]">
            <div className="text-center">
                <span className="inline-flex items-center justify-center gap-2 text-xs font-semibold px-4 py-2 rounded-full mb-6 bg-[#D4A63A]/90 text-[#0F172A] shadow-sm mx-auto">
                    SECURE APPLICATION
                </span>
            </div>
 
            <h2 className="text-3xl font-bold mb-2 text-center text-[#064E3B]">Start Your Loan Application</h2>
            <p className="text-center text-[#059669] mb-6">Completely online, encrypted, takes around 10 minutes.</p>
 
            <div className="relative w-full h-2 bg-[#D1FAE5] rounded-full mb-6">
                <div className="h-2 bg-[#059669] rounded-full transition-all duration-500" style={{ width: `${progressPercent}%` }} />
            </div>
 
            <div className="relative flex justify-between mb-8 pt-1">
                {/* base connecting line */}
                <div className="absolute top-4 left-0 right-0 h-0.5 bg-[#A7DCC8]" />
                {/* animated fill line - grows smoothly as you move between steps */}
                <motion.div
                    className="absolute top-4 left-0 h-0.5 bg-[#059669]"
                    initial={false}
                    animate={{ width: `${(currentStep / (steps.length - 1)) * 100}%` }}
                    transition={{ duration: 0.5, ease: "easeInOut" }}
                />
                {steps.map((step, idx) => (
                    <div key={idx} className="relative z-10 flex flex-col items-center text-center flex-1 px-1">
                        <motion.div
                            className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 mb-2"
                            initial={false}
                            animate={{
                                backgroundColor: idx <= currentStep ? "#059669" : "#FFFFFF",
                                borderColor: idx <= currentStep ? "#059669" : "#A7DCC8",
                                color: idx <= currentStep ? "#FFFFFF" : "#64748B",
                            }}
                        ...
                        <span
                            className={`text-[11px] sm:text-sm font-medium leading-tight transition-colors duration-300 ${currentStep === idx
                                    ? "text-[#022C22] font-bold"
                                    : idx < currentStep
                                        ? "text-[#059669]"
                                        : "text-gray-400"
                                }`}
                        ...
                        <motion.div key="step5" variants={variants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
                            <StepContainer>
                                ...
                                <div className="flex justify-between mt-6">
                                    <button type="button" onClick={handlePrev} className="px-6 py-3 bg-[#D1FAE5] text-[#064E3B] rounded-lg font-medium">Back</button>
                                    <button
                                        type="submit"
                                        disabled={isSubmitting}
                                        className="px-6 py-3 bg-[#059669] text-white rounded-lg hover:bg-[#064E3B] disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2 font-medium"
                                    >
                                        {isSubmitting ? (
                                            <>
...
const Input = ({ type = "text", ...props }: any) => (
    <input
        type={type}
        className="w-full p-3 border-2 border-[#A7DCC8] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#059669] bg-white"
        {...props}
    />
);
...
const NextButton = ({ onClick }: { onClick: () => void }) => (
    <button type="button" onClick={onClick} className="mt-4 px-6 py-3 bg-[#059669] text-white rounded-lg hover:bg-[#064E3B]">Continue</button>
);
 
const BackNext = ({ handlePrev, handleNext }: { handlePrev: () => void; handleNext: () => void }) => (
    <div className="flex justify-between mt-4">
        <button type="button" onClick={handlePrev} className="px-6 py-3 bg-[#D1FAE5] text-[#064E3B] rounded-lg">Back</button>
        <button type="button" onClick={handleNext} className="px-6 py-3 bg-[#059669] text-white rounded-lg hover:bg-[#064E3B]">Continue</button>
    </div>
);
